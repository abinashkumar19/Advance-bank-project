...
locals {
  ...
  generic_tables = concat(local.generic_services, ["webhook-deliveries", "virtual-card-payments"])

  # Every backend microservice (used for ECR repos + IRSA policy).
  # chatbot is stateless (just calls the self-hosted Ollama model server,
  # see below - no DynamoDB table of its own) so it's listed directly
  # here rather than in generic_services. admin-analytics is also
  # stateless in the same sense (reads other services' tables, has none
  # of its own) but still needs an ECR repo/deployment since it's a real
  # running service. ollama itself is NOT here - it runs the official
  # public ollama/ollama image directly (see
  # k8s/services/ollama-deployment.yaml), not something this repo builds,
  # so it needs no ECR repo and isn't part of the Docker build loop.
  ...
  user_indexed_generic_tables = toset([
    ...
    "lockers", "forex", "virtual-card-payments",
  ])
}
