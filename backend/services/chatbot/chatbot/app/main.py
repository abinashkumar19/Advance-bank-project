...
Tradeoff worth knowing: even a larger (7B-class) GPU-hosted model is still
not as reliable at multi-step tool-calling as a hosted frontier model -
...
is rejected outright, see _run_tool). Originally this ran a smaller 3B
model on CPU-only nodes; it now runs on a dedicated GPU node
(terraform/modules/eks - the "gpu" node group) both for speed and because
GPU inference removed the constraint that kept the model small in the
first place, so reliability improved too.
"""
import json
import os
