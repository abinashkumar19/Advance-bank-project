...
# Same SNS topic users-service already publishes registrations to (env var
# name kept for backwards compat - it's a general "notifications" topic in
# practice: notification-writer Lambda fans any message out to both the
# in-app notifications table and Telegram, keyed off the "type" field).
...
def _notify(notif_type: str, user_id: str, summary: str, extra: dict):
    """Publish a notification event for one side of a completed transfer.
    Fans out to the in-app notifications table AND Telegram via
    notification-writer Lambda (see backend/lambdas/notification_writer).
    Best-effort only - the money has already moved by the time this is
    called, so a publish failure here must never surface as an error."""
    try:
        sns_publish(
            SNS_TOPIC_ENV,
            subject=summary,
            message={"type": notif_type, "user_id": user_id, "summary": summary, **extra},
        )
    except Exception as exc:  # noqa: BLE001
        print(f"[transfers] failed to publish {notif_type} notification: {exc}")
...
def create_transfer(req: TransferRequest):
    ...
    # Sender: money debited. Recipient: money credited. Both go to the
    # in-app notifications feed and Telegram (see _notify above) - separate
    # from the email confirmations below, which only fire if an address is
    # known/looked-up.
    _notify(
        "transfer_debit",
        from_acct["user_id"],
        f"₹{req.amount} debited from your account for a transfer to {to_acct.get('owner_name', req.to_account_id)}",
        {"transfer_id": transfer_id, "amount": str(req.amount), "to_account_id": req.to_account_id},
    )
    _notify(
        "transfer_credit",
        to_acct["user_id"],
        f"₹{req.amount} credited to your account from {req.sender_name or 'a VeeraBank user'}",
        {"transfer_id": transfer_id, "amount": str(req.amount), "from_account_id": req.from_account_id, "sender_name": req.sender_name or ""},
    )
