# ---------------------------------------------------------------------------
# RDS (Aurora MySQL) - relational replica of user accounts, fed by the
# users-db-sync Lambda (see the lambda module). Only that Lambda talks to
# this cluster - the users-service pod itself only ever touches DynamoDB.
# ---------------------------------------------------------------------------
...
# Security group attached to the users-db-sync Lambda's ENIs.
...
resource "aws_s3_bucket" "users_db_creds" {
  bucket = "${var.project_name}-${var.environment}-users-db-creds-${var.account_id}"
}
