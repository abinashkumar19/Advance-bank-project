...
async function loadAccounts() {
  ...
  try {
    ...
    panel.innerHTML = `
      ...
      <div class="bank-card fade-in" style="margin-top:16px;">
        <div class="bank-card-top">
          <div class="bank-card-chip"></div>
          <div class="bank-card-mark"><span></span><span></span></div>
        </div>
        <div class="bank-card-number" onclick="navigator.clipboard.writeText('${a.account_number}'); toast('Account number copied.')" title="Click to copy">
          ${formatCardNumber(a.account_number)}
        </div>
        <div class="bank-card-bottom">
          <div class="bank-card-name">${a.owner_name}</div>
          <div class="bank-card-meta">
            <div class="label">Account type</div>
            <div class="value" style="text-transform:capitalize;">${a.account_type}</div>
          </div>
        </div>
      </div>
      <div style="padding:14px 0; margin-top:6px; border-top:1px solid var(--line);">
        <div style="font-size:11px; color:var(--muted-2);">Opened ${fmtWhen(a.created_at)}</div>
      </div>
