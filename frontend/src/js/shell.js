...
// Every page's accent color is picked independently (cycling palette here,
// or a per-page theme's own --ledger hue in css/themes/*.css) - some of
// those are bright/light (gold, teal) where black text reads clearly, and
// some are more saturated blues/violets where black text nearly disappears.
// Rather than hand-picking a text color for every accent everywhere it's
// used (badges, active nav pills, icon tiles, chat bubbles...), compute
// perceived brightness once per accent and expose it as --on-accent, which
// every one of those CSS rules reads instead of a hardcoded black.
function contrastTextFor(hex){
  const n = parseInt(hex.slice(1), 16);
  const r = (n>>16)&255, g = (n>>8)&255, b = n&255;
  // Standard relative luminance (WCAG-style weighting)
  const luminance = (0.299*r + 0.587*g + 0.114*b) / 255;
  return luminance > 0.62 ? "#050308" : "#f6f5fc";
}
function applyAccent(tab){
  ...
  document.documentElement.style.setProperty("--on-accent", contrastTextFor(hex));
}
// Elements inside #main.theme-<page> use that theme's OWN --ledger as
// --accent (see css/themes/*.css), which can differ from the cycling nav
// accent above - so --on-accent needs to be recomputed against the actual
// rendered value for #main specifically, not just inherited from <html>.
function applyMainOnAccent(){
  const main = document.getElementById("main");
  if (!main) return;
  const ledgerHex = getComputedStyle(main).getPropertyValue("--ledger").trim();
  if (/^#[0-9a-fA-F]{6}$/.test(ledgerHex)) {
    main.style.setProperty("--on-accent", contrastTextFor(ledgerHex));
  }
}
...
function renderShell() {
  ...
  document.getElementById("app").innerHTML = `
    <div class="shell">
      ...
      <main id="main" class="theme-${currentTab}"></main>
    </div>
  `;
  renderNav();
  applyMainOnAccent();
